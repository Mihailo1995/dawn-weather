...
const UI = function () {
    ...
    const displayWeatherData = (data, location) => {
        ...
        for (let i = 0; i <= 7; i++) {
            ...
            let maxMinTemp = `<span  id="max-temp">${Math.round((dailyData[i].temperatureMax - 32) * 5 / 9)}&#176;</span> | <span  id="min-temp">${Math.round((dailyData[i].temperatureMin - 32) * 5 / 9)}&#176;</span>`;
        ...
        for (let i = 0; i <= 24; i++) {
            ...
            hourlyWeatherClone.children[2].children[0].innerHTML = Math.round((hourlyData[i].temperature - 32) * 5 / 9) + "&#176;";
